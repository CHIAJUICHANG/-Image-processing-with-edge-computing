...
`define OUT_NUM             3
`define ML0_KERNEL_NUM      36
`define ML1_KERNEL_NUM      288
`define ML2_KERNEL_NUM      288
...
`ifdef p1
  ...
  `define ML0_KERNEL      "../00_TESTBED/PATTERN/ML0_KERNEL.dat"
  `define ML1_KERNEL      "../00_TESTBED/PATTERN/ML1_KERNEL.dat"
  `define ML2_KERNEL      "../00_TESTBED/PATTERN/ML2_KERNEL.dat"
`endif
...
reg  [479:0]  ML0_kernel_mem [0:`ML0_KERNEL_NUM-1];
reg  [479:0]  ML1_kernel_mem [0:`ML1_KERNEL_NUM-1];
reg  [479:0]  ML2_kernel_mem [0:`ML2_KERNEL_NUM-1];
...
integer i, j, k, l, m, n, o, p, q, r, s, t, u, i_mode, pad;
integer ii, jj, kk, o_mode, kkk;
...
initial begin
  ...
  $readmemh(`ML0_KERNEL, ML0_kernel_mem);
	$readmemh(`ML1_KERNEL, ML1_kernel_mem);
  $readmemh(`ML2_KERNEL, ML2_kernel_mem);
  ...
  for(i_mode=0;i_mode<4;i_mode=i_mode+1) begin
    ...
    0: begin
      for(i=0;i<`CONV0_CHANNEL_NUM;i=i+1) begin
        for(j=0;j<74;j=j+1) begin
          for(k=64;k>0;k=k-1) begin
            @(posedge i_clk);
            #(`IN_DEL);
            i_valid = 1'b1;
            if((k<31) && (j<9) && (k>=0)) i_data = {ML0_kernel_mem[i*9+j][16*(k-1)+8+:8], ML0_kernel_mem[i*9+j][16*(k-1)+:8]};
      ...
      for(i=0;i<`CONV1_CHANNEL_NUM;i=i+1) begin
        for(j=0;j<74;j=j+1) begin
          for(k=64;k>0;k=k-1) begin
            ...
            #(`IN_DEL);
            i_valid = 1'b1;
            if((k<31) && (j<36) && (k>=0)) i_data = {ML1_kernel_mem[i*36+j][16*(k-1)+8+:8], ML1_kernel_mem[i*36+j][16*(k-1)+:8]};
            else                          i_data = {16'b0000_0000_0000_0000};
          end
        end
      ...
      for(i=0;i<`CONV2_CHANNEL_NUM;i=i+1) begin
        for(j=0;j<74;j=j+1) begin
          for(k=64;k>0;k=k-1) begin
            ...
            #(`IN_DEL);
            i_valid = 1'b1;
            if((k<31) && (j<72) && (k>=0)) i_data = {ML2_kernel_mem[i*72+j][16*(k-1)+8+:8], ML2_kernel_mem[i*72+j][16*(k-1)+:8]};
            else                          i_data = {16'b0000_0000_0000_0000};
          end
        end
